...
describe("modes", function()
	describe("cycle", function()
		it("should cycle forward through the mode order", function()
			...
			assert.are.equal("mru", modes.cycle("buffers", 1))
			assert.are.equal("files", modes.cycle("mru", 1))
		end)

		it("should cycle backward through the mode order", function()
			assert.are.equal("mru", modes.cycle("files", -1))
			assert.are.equal("files", modes.cycle("buffers", -1))
			assert.are.equal("buffers", modes.cycle("mru", -1))
		end)
